...
namespace ntt {
  ...
  Simulation::Simulation(int argc, char* argv[]) {
    ...
    if (is_resuming) {
      ...
      if (not std::experimental::filesystem::exists(checkpoint_read_path)) {
        raise::Fatal("No checkpoints found", HERE);
      }
      for (const auto& entry :
           std::experimental::filesystem::directory_iterator(checkpoint_read_path)) {
      ...
      if (not std::experimental::filesystem::exists(checkpoint_metafname)) {
        raise::Fatal(
