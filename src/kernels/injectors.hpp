...
namespace kernel {
  ...
  struct GlobalInjector_kernel {
    ...
    const npart_t offset, cntr, domain_idx;
    ...
    Inline void operator()(prtlidx_t p) const {
      ...
      if constexpr (D == Dim::_1D) {
        if (in_x1(p) >= x1_min and in_x1(p) < x1_max) {
          ...
          xi_Cd[0] = static_cast<int>(static_cast<ncells_t>(x_Cd[0]) - i1_offset);
          dxi_Cd[0] = static_cast<prtldx_t>(
            x_Cd[0] - static_cast<real_t>(xi_Cd[0] + i1_offset));
        ...
        if ((in_x1(p) >= x1_min and in_x1(p) < x1_max) and
          ...
          dxi_Cd[0] = static_cast<prtldx_t>(
            x_Cd[0] - static_cast<real_t>(xi_Cd[0] + i1_offset));
          xi_Cd[1] = static_cast<int>(static_cast<ncells_t>(x_Cd[1]) - i2_offset);
          dxi_Cd[1] = static_cast<prtldx_t>(
            x_Cd[1] - static_cast<real_t>(xi_Cd[1] + i2_offset));
        ...
        if ((in_x1(p) >= x1_min and in_x1(p) < x1_max) and
          ...
          dxi_Cd[1] = static_cast<prtldx_t>(
            x_Cd[1] - static_cast<real_t>(xi_Cd[1] + i2_offset));
          xi_Cd[2] = static_cast<int>(static_cast<ncells_t>(x_Cd[2]) - i3_offset);
          dxi_Cd[2] = static_cast<prtldx_t>(
            x_Cd[2] - static_cast<real_t>(xi_Cd[2] + i3_offset));
      ...
      if (should_inject) {
        auto index { Kokkos::atomic_fetch_add(&idx(), 1) };
        // clang-format off
        if (not use_tracking) {
          InjectParticle<M::Dim, M::CoordType, false>(
            index + offset, 
...
namespace experimental {
  template <SimEngine::type S, class M, class ED, class SD1, class SD2>
  struct Injector_with_weights_kernel {
    static_assert(ED::is_energy_dist, "ED must be an energy distribution class");
    static_assert(SD1::is_spatial_dist, "SD1 must be a spatial distribution class");
    static_assert(SD2::is_spatial_dist, "SD2 must be a spatial distribution class");
    ...
    const SD1            spatial_dist1;
    const SD2            spatial_dist2;
    const real_t         inv_V0;
    random_number_pool_t random_pool;

    Injector_with_weights_kernel(real_t                           ppc0,
                              spidx_t                          spidx1,
                              spidx_t                          spidx2,
                              Particles<M::Dim, M::CoordType>& species1,
                              Particles<M::Dim, M::CoordType>& species2,
    ...
    Inline void operator()(index_t i1) const {
      if constexpr (M::Dim == Dim::_1D) {
        ...
        coord_t<Dim::_1D> x_Ph { ZERO };
        metric.template convert<Crd::Cd, Crd::Ph>(x_Cd, x_Ph);
        const auto ppc = static_cast<npart_t>(ppc0 * spatial_dist1(x_Ph));
        if (ppc == 0) {
          return;
        }
        auto rand_gen = random_pool.get_state();
        for (auto p { 0u }; p < ppc; ++p) {
          ...
          if (M::CoordType == Coord::Cart) {
            ...
            weights_1(index + offset1) = wei * spatial_dist2(x_Ph);
            weights_2(index + offset2) = wei * spatial_dist2(x_Ph);
          }
        }
        random_pool.free_state(rand_gen);
      } else {
        raise::KernelError(HERE, "Injector_with_weights_kernel 1D called for 2D/3D");      
      }
    }
    ...
    Inline void operator()(index_t i1, index_t i2) const {
      if constexpr (M::Dim == Dim::_2D) {
        ...
        raise::KernelError(HERE, "Injector_with_weights_kernel 2D called for 1D/3D");
      }
    }
    ...
    Inline void operator()(index_t i1, index_t i2, index_t i3) const {
      if constexpr (M::Dim == Dim::_3D) {
        ...
        raise::KernelError(HERE, "Injector_with_weights_kernel 3D called for 1D/2D");
      }
    }
    }; // struct Injector_with_weights_kernel
  } // namespace experimental
